...
const NewsAddScreen = ({navigation}) => {
 ...
 // const accessToken = 'ya29.a0AbVbY6M7Exw8iT_BSBnBc5NA4hYCBNVVDP6Ja1veGGCCrVCFI4MIR3mCb51Vn6NhvhfUk86HOscwD26CkfutSNKOHjgNF2dwzNVtX0Sjrywu9VLVcyQpgF0UteVo7dnonRfjPLSQ_r7yP4A88nU7zFI5xxD5uVgaCgYKAe4SARISFQFWKvPl4aYEcxFZOic0QdD1G1Mo3A0166';
  useEffect(() => {
    readUserId('user_id');
    getAccessToken();
    ...
    fetch('https://oauth2.googleapis.com/token', {
      ...
      .then(responseJson => {
        ...
        if (responseJson.token_type == 'Bearer') {
          setLoading(false);
          console.log("token generated"+ JSON.stringify(responseJson));  
          const acc_token =  responseJson.access_token;
          setAccessToken(responseJson.access_token);
          console.log("token --"+ acc_token);  
          console.log(Urls.settings+'?data={"data":{"token":"'+acc_token+'"}}');
          fetch(Urls.settings+'?data={"data":{"token":"'+acc_token+'"}}', {
            method: 'POST',
            headers: new Headers({
              'Content-Type': 'application/json',
            }),
  ...
  const selectNewsFile = async () => {
    ...
    if(newsType=='I')
    {
      ...
      ImagePicker.openPicker({
        ...
        //api call for get token from db
        fetch(Urls.contactUs, {
          ...
          .then(responseJson => {
            ...
            if (responseJson.error == 0) {
              ...
              setLoading(false);
              const token_db = responseJson.data.data.accessToken;
              RNFS.stat(image.path).then(fileInfo => {
                ...
                var uploader = new YTDAPI({
                  ...
                  onComplete: function(data) {
                    ...
                    console.log("imagescreenshot---"+data.id);
                    setYoutubeVideoData(data);
                    setVideoId(data.id);
                   // setVideoimageUrl(data.snippet.thumbnails.high.url);
                  }.bind(this)
                });
  ...
  const handleSubmitPress = () => {
    ...
    if(newsFile)
    {
      if(newsType=='I')
      {
        ...
        const newData = [];
        if(youtubeVideoData.status.uploadStatus == 'uploaded')
        {
          let newData = {
            youtube_video_id: videoId,
            embededVideoUrl: videoimageUrl
          }
        }
        else{
        let newData = {
